const { ActionRowBuilder, SelectMenuBuilder ,MessageActionRow,MessageSelectMenu,MessageButton, MessageEmbed} = require('discord.js');
...
module.exports = {
    ...
    run: async (client, message, args) => {
        var avatarmenuid = randomstring.generate(15);
        var bannermenuid = randomstring.generate(14);
        var bilgimenuid = randomstring.generate(16);
    ...
    .addComponents(
        new MessageButton()
          .setCustomId(bilgimenuid)
          .setLabel("Kullanıcı Bilgi")
          .setStyle(4)
      )
    .addComponents(
      new MessageButton()
        .setCustomId(bannermenuid)
        .setLabel("Avatar")
        .setStyle(4)
    )
...
let uid = member.id
    ...
    setTimeout(() => {
        ...
        .setColor(client.warnColor())
         ...
         message.reply({ embeds: [banner2], components: [row] , allowedMentions: { repliedUser: false } });
    }, 1000)
...
client.on('interactionCreate', async interaction => {
    ...
    if (!interaction.isCommand()) {
        if (interaction.customId === bilgimenuid) {
    ...
    if (interaction.customId === bannermenuid) {
